...
This file holds the **frozen v0.2.0 iteration history**. The active release,
v0.3.0, is planned as a dependency graph in [`plan/v0.3.0/`](plan/v0.3.0/README.md)
instead — see the section at the bottom.

## Quick locate (agents read this first)

- **Active release**: **v0.3.0** — **not planned in this file.** Read
  [`plan/v0.3.0/README.md`](plan/v0.3.0/README.md) and run `make v030-status` to
  find the next ready node; the `v030-build` skill executes it.
- **Frozen v0.2.0 reference**: lines 18–491. Historical; do not modify unless explicitly asked.
- **Abandoned FPP plan** (former iterations 15–27): removed 2026-08-15, recoverable via `git show ab37d33:ITERATIONS.md`.
- **Retired**: `PROMPT.md`, the iteration-by-number agent driver, was removed on
  2026-08-15 — the graph plan and the `v030-build` skill replace it. Its
  scope-check convention (split any unit too big for one cheap pass) survives in
  that skill. Recover with `git show ab37d33:PROMPT.md`.
...
# v0.3.0 — planned as a graph, not an iteration list

**The v0.3.0 plan lives in [`plan/v0.3.0/`](plan/v0.3.0/README.md).**

v0.3.0 has genuine parallelism (two backend runtimes, two client platforms) and
a shape that a numbered list misrepresents. It is planned as a dependency graph:
[`plan/v0.3.0/graph.yaml`](plan/v0.3.0/graph.yaml) holds the nodes, their edges,
their model tier and the oracle that proves each one done;
[`plan/v0.3.0/nodes/`](plan/v0.3.0/nodes/) holds one file per node.

```bash
make v030-status
```

Scope: backend choice (AWS **or** self-hosted Docker, one shared core),
WebSocket sync with REST polling retained as a fallback on both backends, and a
connection-status indicator in both apps. See
[`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) for the contract and
[`plan/v0.3.0/decisions.md`](plan/v0.3.0/decisions.md) for the resolved design
questions.

## The abandoned FPP plan

Iterations 15–27 previously described a First Person Project rebuild —
`did:webvh` identities, DIDComm v2.1 through a blind mediator, a personal VTA on
a Raspberry Pi, accountable AI-agent DIDs. **That plan was abandoned on
2026-08-15 and none of it is in scope.** It was removed rather than left in
place, because `ITERATIONS.md` is read by agents as instructions and a stale
plan is worse than no plan. Recover it from git history if it is ever revived:

```bash
git show ab37d33:ITERATIONS.md
```

The one piece that survived is the connection-status indicator, and its state
table was rewritten — the original described a DIDComm session state machine
that does not exist in this design.
