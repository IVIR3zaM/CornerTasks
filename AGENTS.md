...
> **v0.3.0 note:** the three sections below (“Cloud sync”, “Sync model”, “Identity & encryption”) describe the **v0.2.0 stack still on `main`**, and all of it stays true in v0.3.0. What v0.3.0 adds is a **WebSocket transport** (with the REST polling described below retained as a fully-supported fallback) and a **choice of backend** — the same core deployed either as AWS Lambda or as a Docker container the user runs themselves. Identity, encryption, conflict resolution and the archive cutoff are unchanged. See [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) for the contract, [`plan/v0.3.0/`](plan/v0.3.0/README.md) for the build graph, and `docs/connection-status.md` for the status vocabulary. Update these sections in the same PR as the node that changes the behavior.
